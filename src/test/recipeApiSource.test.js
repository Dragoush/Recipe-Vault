...
describe('recipeApiSource', () => {
  ...
  test('lists recipes through the centralized route', async () => {
    ...
    expect(requestJson).toHaveBeenCalledWith('recipes', {
      requiresAuth: true,
      searchParams: {
        page: 3,
        pageSize: 6
  ...
  test('loads statistics through the centralized route', async () => {
    ...
    expect(requestJson).toHaveBeenCalledWith('recipes/statistics', {
      requiresAuth: true
    });
  });
  ...
  test('updates recipes and maps 404s to null', async () => {
    ...
    expect(requestJson).toHaveBeenNthCalledWith(1, 'recipes/recipe-1', {
      method: 'PUT',
      requiresAuth: true,
      body: toRecipeRequestPayload(validRecipeValues)
    });
